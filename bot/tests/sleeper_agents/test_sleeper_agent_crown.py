...
from sleeper_agents.agents.agent_crown import AgentCrown
...
async def test_action_proc_true():
    ...
    mock_message.content = "something something crown"
    agent.message = mock_message

    with patch("sleeper_agents.agents.agent_crown.discord.File") as mock_file:

        result = await agent.action()

        assert result is True

        mock_file.assert_called_once_with("/app/images/crown.gif")
